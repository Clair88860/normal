...
class Dashboard(FloatLayout):
    ...
    def show_gallery(self, *args):
        ...
        scroll = ScrollView()
        # padding oben vergrößern, damit Topbar nicht überlappt
        grid = GridLayout(cols=2, spacing=10, padding=[10, 150, 10, 10], size_hint_y=None)
    ...
    def open_image(self, filename):
        ...
        img = Image(source=path, allow_stretch=True, size_hint=(1,1), pos=(0,0))
        layout.add_widget(img)

        # Arduino Overlay nur wenn aktiviert
        arduino_on = self.store.get("arduino")["value"] if self.store.exists("arduino") else False
        if arduino_on:
            overlay = Label(text="Norden", color=(1,0,0,1), font_size=30,
                            size_hint=(None,None), size=(dp(100), dp(50)),
                            pos=(Window.width*0.05, Window.height*0.8))
            layout.add_widget(overlay)

        # i-Button für Info
        info_btn = Button(text="i", size_hint=(None, None), size=(dp(40), dp(40)),
                          pos=(Window.width - dp(50), Window.height - dp(50)))
    ...
    def show_info(self, filename):
        ...
        # Arduino/Norden nur anzeigen wenn Arduino aktiviert
        arduino_on = self.store.get("arduino")["value"] if self.store.exists("arduino") else False
        if arduino_on:
    ...
    def show_settings(self, *args):
        ...
        layout.add_widget(create_toggle_row("Mit Entzerrung", "winkel"))
